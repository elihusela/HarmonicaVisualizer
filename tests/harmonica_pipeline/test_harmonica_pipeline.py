...
class TestHarmonicaTabsPipeline:
    ...
    def test_pipeline_saves_midi_when_enabled(self, configured_pipeline, save_midi):
        ...
        with patch("harmonica_pipeline.harmonica_pipeline.clean_temp_folder"), patch(
            ...
            configured_pipeline["midi_data"].write.assert_called_once_with(
                pipeline._debug_midi_path
            )

    @pytest.mark.parametrize("save_midi", [False])
    def test_pipeline_skips_midi_write_when_disabled(
        self, configured_pipeline, save_midi
    ):
        pipeline = configured_pipeline["pipeline"]
        pipeline._save_midi = save_midi
    ...
    def test_pipeline_call_order(self, configured_pipeline):
        ...
        with patch("harmonica_pipeline.harmonica_pipeline.clean_temp_folder"), patch(
            ...
            configured_pipeline[
                "pipeline"
            ]._audio_extractor.extract_audio_from_video.assert_called_once()
            mock_predict.assert_called_once()
            configured_pipeline[
                "pipeline"
            ]._tab_mapper.note_events_to_tabs.assert_called_once()
            configured_pipeline[
                "pipeline"
            ]._tab_mapper.save_tabs_to_json.assert_called_once()
            configured_pipeline[
                "pipeline"
