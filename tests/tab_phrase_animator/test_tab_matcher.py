...
def make_parsed_pages(page_data):
    """Helper to convert old int-based test data to ParsedNote format."""
    result = {}
...
class TestTabMatcherValidation:
    ...
    def test_match_empty_midi_tabs(self):
        ...
        parsed_pages = make_parsed_pages({"Page 1": [[[1, 2]]]})
    ...
    def test_match_midi_tabs_none_tabs_attribute(self):
        ...
        parsed_pages = make_parsed_pages({"Page 1": [[[1]]]})
...
class TestTabMatcherBasicMatching:
    ...
    def test_multiple_notes_match(self):
        ...
        parsed_pages = make_parsed_pages({"Page 1": [[[1], [2], [-3]]]})
    ...
    def test_first_match_algorithm(self):
        ...
        midi_tabs = Tabs(
            tabs=[
                TabEntry(tab=1, time=1.0, duration=0.5, confidence=0.8),
                TabEntry(
                    tab=1, time=2.0, duration=0.5, confidence=0.9
                ),  # Same tab, different time
                TabEntry(tab=2, time=3.0, duration=0.5, confidence=0.8),
            ]
        )
        parsed_pages = make_parsed_pages({"Page 1": [[[1], [1], [2]]]})
...
class TestTabMatcherErrorHandling:
    ...
    def test_match_with_zero_confidence_entry(self):
        ...
        midi_tabs = Tabs(
            tabs=[
                TabEntry(
                    tab=1, time=1.0, duration=0.5, confidence=0.0
                ),  # Zero confidence
            ]
        )
        parsed_pages = make_parsed_pages({"Page 1": [[[1]]]})
    ...
    def test_match_with_negative_duration(self):
        ...
        midi_tabs = Tabs(
            tabs=[
                TabEntry(
                    tab=1, time=1.0, duration=-0.1, confidence=0.8
                ),  # Negative duration
            ]
        )
        parsed_pages = make_parsed_pages({"Page 1": [[[1]]]})
