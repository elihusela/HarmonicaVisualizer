...
class TestTabMapper:
    ...
    def test_note_events_to_tabs_valid(self, tab_mapper):
        ...
        assert result.tabs == [
            TabEntry(tab=1, time=0.0, duration=1.0, confidence=0.95),
            TabEntry(tab=-1, time=1.0, duration=0.5, confidence=0.85),
            TabEntry(tab=2, time=2.0, duration=0.6, confidence=0.88),
        ]
    ...
    def test_save_tabs_to_json_calls_open(self, tab_mapper):
        tabs = Tabs(
            [
                TabEntry(tab=1, time=0.0, duration=1.0, confidence=0.5),
                TabEntry(tab=-1, time=1.0, duration=0.5, confidence=0.5),
            ]
        )
